 ...
 Database::Database()
{
	try {
		...
		conn.execute("CREATE TABLE if not exists Profiles (Disc_ID NUMERIC, Profile_ID INTEGER PRIMARY KEY, User_ID NUMERIC)");
		conn.execute("CREATE TABLE if not exists Skip_Chapters (Chapter_Number NUMERIC, Profile_ID NUMERIC, Skip_Chapter_ID INTEGER PRIMARY KEY)");
		conn.execute("CREATE TABLE if not exists Skip_Times (Audio_Only NUMERIC, Profile_ID NUMERIC, Skip_Start NUMERIC, Skip_Stop NUMERIC, Skip_Time_ID INTEGER PRIMARY KEY)");
		conn.execute("CREATE TABLE if not exists Users (Secret_Answer TEXT, Secret_Question TEXT, Can_Play_Unknown NUMERIC, Disc_Rating_ID NUMERIC, Last_Movie_ID NUMERIC, Last_Movie_Position NUMERIC, Password TEXT, User_Icon TEXT, User_ID INTEGER PRIMARY KEY, Username TEXT)");
	}
